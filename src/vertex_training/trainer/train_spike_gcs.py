"""
FAST Spike Test - GCS Parquet Loading
======================================
Loads pre-exported Parquet from GCS (10x faster than BigQuery REST API)
"""
...
def run_test(train_df, val_df, include_spikes):
...
def main():
    ...
    parser.add_argument('--train-prefix', default='spike_test/train/')
    parser.add_argument('--val-prefix', default='spike_test/val/')
    args = parser.parse_args()

    print("="*60)
    print("FAST SPIKE TEST (GCS PARQUET)")
    print("="*60)
    ...
    # Run tests
    baseline = run_test(train_df, val_df, False)
    with_spikes = run_test(train_df, val_df, True)

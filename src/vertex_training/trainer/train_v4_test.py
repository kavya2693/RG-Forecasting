"""
Feature Comparison Test (v4)
============================
Tests 4 configurations:
1. BASELINE - original 31 features
2. BASELINE + SPIKE - 31 + 5 spike features
3. BASELINE + VELOCITY - 31 + 4 velocity/gap features
4. BASELINE + ALL - 31 + 5 + 4 = 40 features
"""
...
# Note: feat_velocity_segment is categorical, handled separately
...
def train_model(train_df, val_df, features, segment, params):
    ...
    # Add velocity_segment as categorical if requested
...
def run_test(train_df, val_df, feature_config, label):
    print(f"\n{'='*50}\n  {label}\n{'='*50}")

    # Build feature list based on config
    features = ORIGINAL_NUMERIC.copy()
    if 'spike' in feature_config:
        features += SPIKE_FEATURES
    if 'velocity' in feature_config:
...
def main():
    ...
    parser.add_argument('--train-prefix', default='spike_test_v4/train/')
    parser.add_argument('--val-prefix', default='spike_test_v4/val/')
    args = parser.parse_args()

    print("="*60)
    print("FEATURE COMPARISON TEST (v4)")
    print("="*60)
    ...
    results = {}
    results['baseline'] = run_test(train_df, val_df, [], "1. BASELINE (31 features)")
    results['spike'] = run_test(train_df, val_df, ['spike'], "2. BASELINE + SPIKE (36 features)")
    results['velocity'] = run_test(train_df, val_df, ['velocity'], "3. BASELINE + VELOCITY (35 features)")
    results['all'] = run_test(train_df, val_df, ['spike', 'velocity'], "4. BASELINE + ALL (40 features)")
    ...
    print(f"\n{'Config':<25} {'Daily WFA':>12} {'Weekly WFA':>12} {'Δ Daily':>10} {'Δ Weekly':>10}")
    print("-"*70)
    for name, metrics in results.items():
        d_delta = metrics['daily_wfa'] - baseline_daily
        w_delta = metrics['weekly_store_wfa'] - baseline_weekly
        print(f"{name:<25} {metrics['daily_wfa']:>11.2f}% {metrics['weekly_store_wfa']:>11.2f}% {d_delta:>+9.2f}pp {w_delta:>+9.2f}pp")

    # Find best config
    best_config = max(results.items(), key=lambda x: x[1]['daily_wfa'])
    print(f"\n{'='*70}")
    print(f"BEST CONFIG: {best_config[0].upper()} (+{best_config[1]['daily_wfa'] - baseline_daily:.2f}pp daily WFA)")
    print("="*70)
    ...
    output = {
        ...
        'improvement_daily_pp': best_config[1]['daily_wfa'] - baseline_daily,
        'improvement_weekly_pp': best_config[1]['weekly_store_wfa'] - baseline_weekly,
        'timestamp': datetime.now().isoformat()
    }
    with open('/tmp/v4_test_results.json', 'w') as f:
        json.dump(output, f, indent=2)
    print("\nResults saved to /tmp/v4_test_results.json")
